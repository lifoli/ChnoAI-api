...
def create_app():
    ...
    # url -> conversation_id, tech_note
    # TODO - main-server의 api로 대체 필요... 
    @app.route('/create/link', methods=['POST'])
    def create_tech_note_from_link():
        data = request.get_json()
        user_id = data.get("userId")
        url = data.get("url")

        if not user_id or not url:
            return jsonify({"message": "Missing required fields: userId or url"}), 400

        print("작업 시작")
        try:
            conversation_response = database.table("conversations").insert([
                {
                    "user_id": user_id,
                    "source": "direct_link",
                    "link": url,
            ...
            if hasattr(conversation_response, 'data'):
                conversation_data = conversation_response.data[0]
            else: 
                return jsonify({"message": "Error inserting conversation"}), 500

            
            chat_url, chat_room_title, data = run_headless_browser(url)
            conversation = {"chatUrl": chat_url, "chatRoomTitle": chat_room_title, "data": data}

            messages = [
                {
                    "message_type": "question",
                    "message_content": item["question"],
                    "conversation_id": conversation_data["id"],
                    "sequence_number": index * 2 + 1,
                } for index, item in enumerate(conversation['data'])
            ] + [
                {
                    "message_type": "answer",
                    "message_content": item["answer"],
                    "conversation_id": conversation_data["id"],
                    "sequence_number": index * 2 + 2,
                } for index, item in enumerate(conversation['data'])
            ]

            tech_note_response = database.table("tech_notes").insert([
                {
                    "conversation_id": conversation_data["id"],
            ...
            if hasattr(tech_note_response, 'data'):
                tech_note_data = tech_note_response.data[0]
            else:
                return jsonify({"message": "Failed to insert tech note"}), 500
            
            messages_response = database.table("messages").insert(messages).execute()

            if not hasattr(messages_response, 'data'):
                return jsonify({"message": "Failed to insert messages"}), 500


            return jsonify({"techNoteData": tech_note_data}), 201
